---
title: 'Долгие гласные'
order: 30
---
<div class="theory-hero">
  <div class="theory-hero-inner">
    <div class="theory-hero-letter">
      ا و ى
    </div>
    <div class="theory-hero-content">
      <h1 class="theory-hero-title">
        Долгие гласные
      </h1>
      <div class="theory-hero-sub">
        Буквы <b>ا</b>, <b>و</b> и <b>ى</b> без огласовок служат для обозначения <b>долгих гласных звуков</b>.
        Каждой из них предшествует соответствующая огласовка: алифу — фатха, уау — дамма, йа — кясра.
      </div>
    </div>
  </div>
</div>

<section class="theory-section">
<h2 class="theory-heading">
  Три буквы удлинения
</h2>

<div class="theory-card">
  <div class="rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-white/[0.06] dark:bg-white/[0.02]">
    <div class="flex items-center gap-4">
      <div class="arab-mark shrink-0 text-4xl" style="color: var(--color-fjord);">ءَا</div>
      <p class="!my-0 leading-relaxed">
        Если перед <b>ا</b> приходит знак «фатха» (ــَـ), то это сочетание читается протяжно, как два «Ә»:
        <span class="arab">ءَابَاتَا</span>
      </p>
    </div>
  </div>
  <div class="mt-3 rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-white/[0.06] dark:bg-white/[0.02]">
    <div class="flex items-center gap-4">
      <div class="arab-mark shrink-0 text-4xl" style="color: var(--color-eggplant);">إِى</div>
      <p class="!my-0 leading-relaxed">
        Если перед <b>ى</b> без огласовки приходит знак «кясра» (ــِـ), то это сочетание читается протяжно, как два «и».
      </p>
    </div>
  </div>
  <div class="mt-3 rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-white/[0.06] dark:bg-white/[0.02]">
    <div class="flex items-center gap-4">
      <div class="arab-mark shrink-0 text-4xl" style="color: var(--color-forest);">أُو</div>
      <p class="!my-0 leading-relaxed">
        Если перед <b>و</b> без огласовки приходит знак «дамма» (ــُـ), то это сочетание читается протяжно, как два «у»:
        <span class="arab">أُوبُوتُو</span>
      </p>
    </div>
  </div>

  <div class="mt-4 rounded-xl border border-amber-200 bg-amber-50/50 p-4 dark:border-amber-800/40 dark:bg-amber-900/10">
    <p class="mb-2 text-xs font-semibold uppercase tracking-widest text-amber-600 dark:text-amber-400">✨ Интересный пример</p>
    <p class="!mb-0 text-sm leading-relaxed text-gray-700 dark:text-gray-300">
      Все три буквы «мад» в одном слове — <span class="arab">وَأُوتِينَا</span> («и нам даровано») в аяте суры «Ан-Намль»:
    </p>
  </div>
  <div class="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-6 dark:border-emerald-900/40 dark:bg-emerald-950/20">
    {Quran}27:16{/Quran}
  </div>
</div>
</section>

<section class="theory-section">
<h2 class="theory-heading">
  Как произносить долгие гласные
</h2>

<div class="theory-card">
  <ul>
    <li>
      Во время произнесения долгой гласной <b>ا</b> язык ровный, есть колебания в горле. Рот должен открываться.
    </li>
    <li>
      Во время произнесения <b>و</b> задняя часть языка приподнята, а губы вытянуты.
    </li>
    <li>
      Во время произнесения <b>ى</b> язык приподнят в середине, и нижняя челюсть опускается.
    </li>
  </ul>

  <div class="mt-4 rounded-xl border border-red-100 bg-red-50/30 p-4 dark:border-red-900/30 dark:bg-red-900/10">
    <p class="mb-2 text-xs font-semibold uppercase tracking-widest text-red-500 dark:text-red-400">❌ Избегайте гунны</p>
    <p class="!mb-0 text-sm leading-relaxed text-gray-700 dark:text-gray-300">
      Если соблюдать эти правила, воздух не будет запускаться в нос (не будет гунны).
      <b>Проверка:</b> закройте нос и произнесите звук. Если произносите правильно, закрытый нос не помешает произношению.
      Если мешает — значит, воздух идёт в нос, и это неправильно.
    </p>
  </div>
</div>
</section>
