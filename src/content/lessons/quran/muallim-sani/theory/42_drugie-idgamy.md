---
title: 'Другие виды идгама'
order: 43
---
<div class="theory-hero">
  <div class="theory-hero-inner">
    <div class="theory-hero-letter">
      ـّـ
    </div>
    <div class="theory-hero-content">
      <h1 class="theory-hero-title">
        Другие виды идгама
      </h1>
      <div class="theory-hero-sub">
        Слияние происходит не только после <span class="arab">نۡ</span> и <span class="arab">مۡ</span>:
        сливаются также <b>одинаковые</b>, <b>подобные</b> и <b>близкие</b> по месту образования буквы.
      </div>
    </div>
  </div>
</div>

<section class="theory-section">
<h2 class="theory-heading">
  Три вида идгама
</h2>

<div class="theory-card">
  <div class="overflow-x-auto">
    <table class="w-full text-center">
      <thead>
        <tr>
          <th class="p-2">Мутамасилайн<br/>(одинаковые)</th>
          <th class="p-2">Мутажанисайн<br/>(подобные)</th>
          <th class="p-2">Мутакарибайн<br/>(близкие)</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td class="p-2">все одинаковые буквы, кроме <span class="arab">م</span> и <span class="arab">ن</span></td>
          <td class="p-2"><span class="arab">د ت ط</span><br/><span class="arab">ث ذ ظ</span><br/><span class="arab">ب م</span></td>
          <td class="p-2"><span class="arab">ل ر</span><br/><span class="arab">ق ك</span></td>
        </tr>
      </tbody>
    </table>
  </div>
</div>
</section>

<section class="theory-section">
<h2 class="theory-heading">
  Идгам мутамасилайн — одинаковые буквы
</h2>

<div class="theory-card">
  <p class="!mt-0 leading-relaxed">
    При встрече двух <b>одинаковых</b> букв, первая из которых с сукуном, а вторая
    с огласовкой, первая буква поглощается второй с удвоением:
  </p>
  <p class="text-center"><span class="arab text-2xl">أَنِ ٱضۡرِب بِّعَصَاكَ</span> <span class="text-sm text-gray-500">(фрагмент из 7:160)</span></p>
  <p class="text-center"><span class="arab text-2xl">ءَاوَواْ وَّنَصَرُوٓاْ</span> <span class="text-sm text-gray-500">(фрагмент из 8:72)</span></p>
  <p class="leading-relaxed">
    <b>Подсказка в Коране:</b> отсутствие «сукуна» над первой и наличие «шадды»
    над второй буквой.
  </p>
  <p class="leading-relaxed">
    <b>Примечание:</b> при встрече <span class="arab">م</span> с <span class="arab">م</span>
    и <span class="arab">ن</span> с <span class="arab">ن</span> это правило не действует —
    эти буквы произносятся с назализацией.
  </p>
  <div class="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-6 dark:border-emerald-900/40 dark:bg-emerald-950/20">
    {Quran}2:16{/Quran}
  </div>
  <div class="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-6 dark:border-emerald-900/40 dark:bg-emerald-950/20">
    {Quran}4:63{/Quran}
  </div>
</div>
</section>

<section class="theory-section">
<h2 class="theory-heading">
  Идгам мутажанисайн — подобные буквы
</h2>

<div class="theory-card">
  <p class="!mt-0 leading-relaxed">
    При встрече двух <b>схожих по месту образования</b> букв, где первая с сукуном,
    а вторая с огласовкой, вторая ассимилирует (поглощает) первую с удвоением.
  </p>
  <p class="leading-relaxed">
    Группа <span class="arab">د ت ط</span>. Правило действует для пар:
    <span class="arab">ت</span> после <span class="arab">د</span>,
    <span class="arab">د</span> после <span class="arab">ت</span>,
    <span class="arab">ط</span> после <span class="arab">ت</span>,
    <span class="arab">ت</span> после <span class="arab">ط</span>:
  </p>
  <p class="text-center"><span class="arab text-2xl">فَرَّطتُمۡ ، وَقَالَت طَّآئِفَةٌ</span></p>
  <div class="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-6 dark:border-emerald-900/40 dark:bg-emerald-950/20">
    {Quran}109:4{/Quran}
  </div>
<div class="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-5 dark:border-amber-900/40 dark:bg-amber-950/20">
  <p class="!mt-0 leading-relaxed">
    <strong>Примечание.</strong>
    В паре 
    <span class="arab">ت</span> после <span class="arab">ط</span>,
    как в примере
    <span class="arab">فَرَّطتُمۡ</span>,
    ассимиляция является
    <strong>неполной (إدغام ناقص — идгам накис)</strong>.
  </p>

  <p class="mb-0 leading-relaxed">
    Сильная буква
    <span class="arab">ط</span>
    не поглощается полностью: её
    <strong>твердость (тафхим) сохраняется</strong>.
    Поэтому над буквой
    <span class="arab">ت</span>
    <strong>шадда не ставится</strong>.
    В остальных парах этой группы слияние является
    <strong>полным</strong>, поэтому шадда обязательна,
    например:
    <span class="arab">وَقَالَت طَّآئِفَةٌ</span>.
  </p>
</div>

</div>

<div class="theory-card mt-4">
  <p class="!mt-0 leading-relaxed">
    Группа <span class="arab">ث ذ ظ</span>. Правило действует для пар:
    <span class="arab">ذ</span> после <span class="arab">ث</span>,
    <span class="arab">ظ</span> после <span class="arab">ذ</span>:
  </p>
  <p class="text-center"><span class="arab text-2xl">إِذ ظَّلَمُوٓاْ ، يَلْهَث ذَّٰلِكَ</span></p>
  <p class="leading-relaxed">
    Единственный пример в Коране, когда <span class="arab">ذ</span> идёт после
    <span class="arab">ث</span> — фрагмент аята 7:176:
  </p>
  <p class="text-center"><span class="arab text-2xl">يَلۡهَثۡ ذَّٰلِكَ مَثَلُ ٱلۡقَوۡمِ ٱلَّذِينَ كَذَّبُواْ بِ‍َٔايَٰتِنَا</span></p>
</div>

<div class="theory-card mt-4">
  <p class="!mt-0 leading-relaxed">
    Группа <span class="arab">ب م</span>. Правило действует для пары:
    <span class="arab">م</span> после <span class="arab">ب</span>.
  </p>
  <div class="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-6 dark:border-emerald-900/40 dark:bg-emerald-950/20">
    {Quran}11:42{/Quran}
  </div>
    <p class="leading-relaxed">
    <b>Подсказка в Коране:</b> отсутствие «сукуна» над первой и наличие «шадды»
    над второй буквой.
  </p>
</div>
</section>

<section class="theory-section">
<h2 class="theory-heading">
  Идгам мутакарибайн — близкие буквы
</h2>

<div class="theory-card">
  <p class="!mt-0 leading-relaxed">
    При встрече двух <b>близких по месту образования</b> букв, где первая с сукуном,
    а вторая с огласовкой, вторая ассимилирует (поглощает) первую с удвоением.
  </p>
  <p class="leading-relaxed">
    Пара <span class="arab">ل ر</span>: <span class="arab">ر</span> после
    <span class="arab">ل</span>:
  </p>
  <div class="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-6 dark:border-emerald-900/40 dark:bg-emerald-950/20">
    {Quran}20:114{/Quran}
  </div>
  <p class="leading-relaxed">
    <b>Исключение</b> — здесь стоит «сакта», поэтому слияния нет:
  </p>
  <div class="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-6 dark:border-emerald-900/40 dark:bg-emerald-950/20">
    {Quran}83:14{/Quran}
  </div>
  <p class="leading-relaxed">
    <b>Подсказка в Коране:</b> отсутствие «сукуна» над первой и наличие «шадды»
    над второй буквой. В исключении — наличие «сукуна» над первой и отсутствие
    «шадды» над второй буквой.
  </p>
</div>

<div class="theory-card mt-4">
  <p class="!mt-0 leading-relaxed">
    Пара <span class="arab">ق ك</span>: <span class="arab">ك</span> после
    <span class="arab">ق</span>. Единственный пример в Коране:
  </p>
  <div class="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-6 dark:border-emerald-900/40 dark:bg-emerald-950/20">
    {Quran}77:20{/Quran}
  </div>
    <p class="leading-relaxed">
    <b>Подсказка в Коране:</b> отсутствие «сукуна» над первой и наличие «шадды»
    над второй буквой.
  </p>
</div>
</section>
